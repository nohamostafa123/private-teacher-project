...
const TeacherDetails = () => {
    ...
    const fetchTeacherDetails = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/teachers/list/${id}`
        );
  ...
  const handleRatingClick = async (value) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/teachers/rate/${id}`,
        { rating: value }
      );
      ...
      await Swal.fire({
        icon: "success",
        title: "Success",
        text: response.data.message || "Rating submitted successfully!",
      });
  ...
  //     try {
  //         const response = await axios.post(`/api/teachers/rate/${id}`, { rating: value });
  ...
  //             alert(`Error: ${error.response.data.message || 'Error submitting rating. Please try again later.'}`);
  //         } else {
      ...
      <div className="big">
        <div className="myTeacherHeader">
              ...
              {activeTab === "info" && (
                <div className="teacher-metaa two-columns">
                  <div className="teacher-meta-item">
                    <FaUserAlt className="icon" /> الاسم:{" "}
                  ...
                  <div className="teacher-meta-item">
                    <FaFlag className="icon" /> بلد الإقامة:{" "}
                    {teacher.country || "Egypt"}
                  </div>
                  <div className="teacher-meta-item">
                    <FaCalendarAlt className="icon" />
                    تاريخ الميلاد : {teacher.date_of_birth || "N/A"}
                  </div>
                  <div className="teacher-meta-item">
                    <FaBriefcase className="icon" /> سنوات الخبرة:{" "}
                    {teacher.years_of_experience || "N/A"}
                  </div>
                  <div className="teacher-meta-item">
                    <FaGenderless className="icon" /> النوع :{" "}
                    {teacher.gender || "N/A"}
                  </div>
                </div>
              )}
              ...
              <div className="mb-3">
                ...
                {[1, 2, 3, 4, 5].map((value) => (
                  <FaStar
                    key={value}
                    className={`star-icon ${
                      rating >= value ? "text-warning" : "text-muted"
                    }`}
          ...
          <div className="profile-tab-container ">
            ...
            <span
              className={`profile-tab-item ${
                activeProfileTab === "options" ? "active" : ""
              }`}
              onClick={() => setActiveProfileTab("options")}
            >
              الحالة
            </span>
          </div>
