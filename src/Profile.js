...
const Profile = () => {
  ...
  if (file) {
    // استخدام FileReader لقراءة الملف وعرضه
    const reader = new FileReader();
    reader.onloadend = () => {
      setSelectedImage(reader.result); // تحديث الصورة المحددة
    };
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = userType === 'teacher'
      ...
      const updatedFormData = new FormData(); // استخدم FormData لإرسال الصورة
      Object.keys(formData).forEach(key => {
        updatedFormData.append(key, formData[key]);
      });
      if (selectedImage) {
        updatedFormData.append('profilePicture', formData.profilePicture); // إضافة الصورة إلى البيانات
      }
  
      try {
        const response = await axios.put(endpoint, updatedFormData, {
          headers: {
            'Content-Type': 'multipart/form-data' // استخدم multipart/form-data لإرسال الصور
          }
        });
    ...
    <div className="bg-profile">
      <div className="profile-page">
        ...
        <div className="profile-header">
          <div className="avatar-container">
            <img
               src={selectedImage || user.profilePicture || user.image}
              alt="Profile"
              className="profile-avatar"
            />
             <label className="camera-icon">
